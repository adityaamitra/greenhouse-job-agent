...
def main():
    ...
    print(
        f"CAPTCHA challenges:      {summary.get('challenge_count', 0)}"
    )
    print(
        f"Browser-modified jobs:   {summary.get('browser_modified_count', 0)}"
    )
    print(
        f"Runtime:                 {float(report.get('total_seconds') or 0):.2f}s"
    )

    if args.persist:
        print(
            "Queue history persisted: "
            f"{'YES' if report.get('history_persisted') else 'NO'}"
        )
